...
# Regex helpers for aggregate() fallbacks
DOC_CLASS_MISSING_RX = re.compile(r"File `([^`]+\.cls)' not found")
PACKAGE_MISSING_RX = re.compile(r"File `([^`]+)\.sty' not found")
...
def _assemble_document(plan: Dict, snippets_dir: Path, evidence_log: Path) -> AssembleResult:
    ...
    doc_class_options = plan.get("doc_class_options")
...
def _build_main(doc_class: str, doc_class_options: str | None, sections: List[SectionEntry], used_bib: bool) -> Tuple[str, List[SourceMapEntry]]:
    ...
    opts = (doc_class_options or "").strip()
    docclass_line = f"\\documentclass[{opts}]{{{doc_class}}}" if opts else f"\\documentclass{{{doc_class}}}"
    lines: List[str] = [
        docclass_line,
        "\\input{preamble.tex}",
        "",
...
def _write_main(doc_class: str, doc_class_options: str | None, sections: List[SectionEntry], used_bib: bool,
                out_dir: Path, evidence_log: Path) -> Tuple[Path, List[SourceMapEntry]]:
    out_dir.mkdir(parents=True, exist_ok=True)
    main_tex = out_dir / "main.tex"
    tex, mapping = _build_main(doc_class, doc_class_options, sections, used_bib)
...
def run_aggregator(plan_path: str, snippets_dir: str, out_dir: str,
    ...
    main_tex, mapping = _write_main(res.doc_class, res.doc_class_options, res.sections, res.used_bib, out_dir_path, evidence_log)
...
def aggregate(
    ...
    *,
    ...
    """High-level helper used in tests to run aggregation with basic fallbacks."""
    plan_file = Path(plan_path)
    ...
    if compile_pdf:
        compile_attempted = True
        compile_ok, stdout, stderr = _run_latexmk(Path(result["main_tex"]), result["engine"], verbose)
    result.update({"compile_attempted": compile_attempted, "compile_ok": compile_ok, "stdout": stdout, "stderr": stderr})
    status = "compiled_ok" if compile_ok else ("skipped" if not compile_pdf else "failed")
    if compile_pdf and not compile_ok:
        stderr = stderr or ""
        class_match = DOC_CLASS_MISSING_RX.search(stderr)
        if class_match:
            fallback_plan = dict(plan_data)
            fallback_plan["doc_class"] = "scrartcl"
            fallback_plan["doc_class_options"] = fallback_plan.get("doc_class_options") or "11pt"
            tmp_plan = Path(out_dir) / "plan.fallback.json"
            tmp_plan.write_text(json.dumps(fallback_plan, ensure_ascii=False, indent=2), encoding="utf-8")
            result = run_aggregator(
                tmp_plan,
                Path(snippets_dir),
                Path(out_dir),
            ...
            ok, stdout, stderr = _run_latexmk(Path(result["main_tex"]), result["engine"], verbose)
            result.update({"compile_attempted": True, "compile_ok": ok, "stdout": stdout, "stderr": stderr})
            result["status"] = "ok_fallback" if ok else "failed"
            return result
        missing_pkg = _extract_missing_package(stderr)
        if missing_pkg:
            preamble_path = Path(out_dir) / "preamble.tex"
            if _prune_package(preamble_path, missing_pkg):
                ok, stdout, stderr = _run_latexmk(Path(result["main_tex"]), result["engine"], verbose)
                result.update({"compile_ok": ok, "stdout": stdout, "stderr": stderr, "compile_attempted": True})
                status = "compiled_ok" if ok else "failed"
            else:
                status = "failed"
...
def _extract_missing_package(stderr: str) -> str | None:
    match = PACKAGE_MISSING_RX.search(stderr or "")
    if not match:
        return None
    return match.group(1)


def _prune_package(preamble_path: Path, package: str) -> bool:
    if not preamble_path.exists():
        return False
    pattern = re.compile(rf"\\usepackage(?:\[[^]]+\])?\{{{re.escape(package)}\}}")
    lines = preamble_path.read_text(encoding="utf-8").splitlines()
