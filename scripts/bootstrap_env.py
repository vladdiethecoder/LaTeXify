...
def _locate_installer() -> Path:
    candidates = [
        REPO_ROOT / "src" / "latexify" / "scripts" / "install_models.py",
...
def _verify_kimi_model() -> None:
    ...
    if model_path is None:
        ...
        return
    try:
        from latexify.models.kimi_k2_adapter import GGUFModelConfig, KimiK2InstructAdapter
...
def _verify_vllm_client() -> None:
    if os.environ.get("LATEXIFY_DISABLE_VLLM", "0") == "1":
        ...
        return
    try:
        from latexify.models.vllm_client import get_vllm_client
