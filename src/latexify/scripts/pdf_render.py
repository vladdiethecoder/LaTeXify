...
This is a package-scoped variant of the top-level :mod:`scripts.pdf_render`
module so that tests and external tools can rely on
``latexify.scripts.pdf_render`` in both editable and installed contexts.
"""
...
# When imported as ``latexify.scripts.pdf_render``, we resolve templates from
# the package-local ``templates`` directory.
