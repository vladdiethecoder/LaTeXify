...
from release.pipeline import reward, reward_mm
...
def test_reward_evaluation(tmp_path):
    ...
    reward.evaluate_rewards(chunks_path, tex_path, validation_path, reward_path)
...
def test_reward_modes_and_trace(tmp_path, monkeypatch):
    ...
    def fake_mm_score(_tex_path):
        return 0.42

    monkeypatch.setattr(reward_mm, "aesthetic_mm_score", fake_mm_score)
